...
def require_csrf(request: Request, token: Optional[str]) -> None:
    ...
    # Tokens are URL-safe base64: anything non-ASCII (even a lone surrogate) is forged, and must not reach encode() -> 500
    if not expected or not token or not str(token).isascii() or not hmac.compare_digest(
        str(expected).encode("ascii"), str(token).encode("ascii")
    ):
