...
>
> Этот файл — единый источник правды для всей команды: стек, структура репозитория, маршруты, схема БД, i18n, контракты API, тайминг 7 фаз и правила сдачи. Детали каждой фазы — в `docs/PHASE_1..7_*.md`. Задача для второго участника — `docs/ISSUE_i18n_and_minor_tasks.md`. Все факты о данных и правилах — `docs/01_REFERENCE.md`.
...
| GET | `/app` | **все (гость тоже)** | 4 | Форма подбора; с query-параметрами — результаты |
...
## 9. Дизайн-система (CSS-переменные, `static/css/app.css`)

```css
:root {
  --c-bg: #F7F8FA; --c-surface: #FFFFFF; --c-text: #10151C; --c-muted: #5B6573;
  --c-border: #E3E7ED;
  --c-primary: #0096B7;   /* небесно-голубой */
  --c-primary-ink: #FFFFFF;
  --c-accent: #E8B100;    /* золото */
  --c-ok: #1B8A4B; --c-warn: #B76E00; --c-bad: #C62828; --c-info: #2F6FDB;
...
@media (prefers-color-scheme: dark) {
  :root { --c-bg:#0E1216; --c-surface:#161B22; --c-text:#E8EDF2; --c-muted:#9AA6B2; --c-border:#262D36; }
}
```

Компоненты (классы): `.btn`, `.btn--primary`, `.btn--ghost`, `.card`, `.badge`, `.badge--synthetic`, `.badge--imputed`, `.banner--found|--partial|--nocat|--none|--invalid`, `.funnel`, `.form-grid`, `.field`, `.field__error`, `.chips`, `.calendar`, `.container` (max-width 1120px, отступы 16px на мобильных).

Статус-баннеры: `found` — зелёный, `partial` — жёлтый, `no_category_in_city` — синий, `none_match` — красный, `invalid_request` — серый. **Цвет никогда не единственный носитель смысла**: у каждого баннера иконка и текст.
...
2. Скопировать промпт из раздела «Промпты для Codex» и дать агенту вместе с этим мастер-планом и `01_REFERENCE.md`.
3. Проверить по «Критериям приёмки», прогнать тесты фазы, закоммитить с сообщением из раздела «Коммиты».
4. Если фаза не влезает в тайм-бокс — смотреть раздел «Что режем» этой фазы.
